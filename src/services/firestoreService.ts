...
import { PomodoroDataType, ProjectTimerDataType, SelectListDataType, TodoTimerDataType, TogetherDataType } from "../types/alarm";
...
export const getEventsFromFirestore = async (): Promise<NewEventStateType[]> => {
    try {

        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('events').where('uid', '==', userId).get();
        const events: NewEventStateType[] = snapshot.docs.map(doc => {
            ...
            return {
                ...
                date: new Date(data.year, data.month - 1, data.day, data.hour, data.minut, data.second).toISOString(),
...
export const fetchTasksFromFirestore = async (): Promise<TodoTimerDataType[]> => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
...
export const getAllCountriesFromFirestore = async () => {
    try {
        ...
        const userId = user.uid;
        console.log('userIduserIduserIduserId', userId);
        const snapshot = await db.collection('countries').where('uid', '==', userId).get();
...
export const getSectorsFromFirestore = async () => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('sectors').where('uid', '==', userId).get();
...
export const getEtapsFromFirestore = async () => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('etaps').where('uid', '==', userId).get();
...
export const getPomodoroTasksFromFirestore = async () => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('pomodoroTasks').where('uid', '==', userId).get();
...
export const getAlarmsFromFirestore = async () => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('alarms').where('uid', '==', userId).get();
...
// Project Timer
export const addProjectTimerToFirestore = async (timer: ProjectTimerDataType) => {
    try {
...
export const fetchProjectTimersFromFirestore = async (): Promise<ProjectTimerDataType[]> => {
    try {
        const user = auth().currentUser;
        const userId = user.uid;
        const snapshot = await db.collection('projectTimers').where('uid', '==', userId).get();
...
export const updateProjectTimerInFirestore = async (id: any, updatedTimer: any) => {
    try {
